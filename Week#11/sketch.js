  ...
  move() {
    ...
    for (let obj of immovableObjects) {
      if (obj.collidesWith(nextX, nextY, 100)) {
        blocked = true;
        particles.push(new ParticleSystem(this.x + 50, this.y + 50));
        break;
      }
    }
...
function draw() {
  ...
  // 🎉 Win condition if all good items are gone
  ...
  // 💀 Lose logic (obstacle removed if health hits zero)
  if (health <= 0) {
    if (immovableObjects.length > 0) {
      immovableObjects.pop();
  ...
  for (let obj of immovableObjects) obj.display();
  for (let item of goodItems) item.display();
  for (let item of badItems) item.display();
